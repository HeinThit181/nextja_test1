...
export async function PUT(req, { params }) {
    ...
    const updateDoc = {};
    if (data.name != null) updateDoc.itemName = data.name;
    if (data.category != null) updateDoc.itemCategory = data.category;
    if (data.price != null) updateDoc.itemPrice = data.price;
    try {
        const client = await getClientPromise();
        const db = client.db("wad-01");
        const updatedResult = await db.collection("item").updateOne({
...
export async function DELETE(req, { params }) {
    ...
    try {
        const client = await getClientPromise();
        const db = client.db("wad-01");
        const deleteResult = await db.collection("item").deleteOne({
            _id: new ObjectId(id),
        });
